...
const appNav = css`grid-area: nav; display: flex; flex-flow: column nowrap;`;
...
function App() {
  ...
  return (
    <div css={appCss}>
      ...
      <nav css={appNav}>
        {CHARTS.map(({ name, key }) => (<a css={[linkStyle, key === chart && tw`underline`]} key={key} href={`#${key}`} onClick={() => setChart(key)}>{name}</a>))}
        <div>Sankey</div>
        <div>SPLOM</div>
        <div>Sparklines</div>
        <div>Hexbins</div>
        <hr />
        <div><a css={linkStyle} href="https://github.com/kdivringi/visx-rodeo">(repo)</a></div>
      </nav>
      <article css={css({gridArea: 'content'})}>{selectedChart && (
        <ParentSize>
